...
{
	TextBlock_ButtonSignOut->SetColorAndOpacity(HoveredTextColor);
}

void UAccountDropdown_Expanded::SignOutButton_UnHover()
{
	TextBlock_ButtonSignOut->SetColorAndOpacity(UnHoveredTextColor);
}
...
void UAccountDropdown_Expanded::SetSignOutButtonStyleTransparent()
{
	FButtonStyle Style;
	FSlateBrush Brush;
	Brush.TintColor = FSlateColor(FLinearColor(0.f,0.f,0.f));  //Button is Transparent
